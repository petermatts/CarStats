...
class Subaru_Corrections(Correction_Template):
	"""
	Helper class for Subaru corrections
	"""
		...
		for k in data.keys():
			match k:
				...
				case "Trim":
					...
					result[k] = result[k].replace("Ed", "Edition")
					result[k] = result[k].replace("Ltd", "Limited")
					result[k] = re.sub(r"(AT|MT) ", "", result[k])

					if result['Model'] == "Outback":
						result[k] = result[k].replace("Outback ", "")
					elif result['Model'] == "WRX":
						result[k] = result[k].replace("WRX ", "")

					t = re.search(r"((\d-Pass(enger)?|XT| X |GT|Wilderness|Touring|Sports?|Turbo|WRX|Outback|Limited|Premium|Special|L.L.Bean|Black|Edition|Series\.(Gray|White|(Hyper)?Blue|Yellow)) ?)+", result[k])
					if t is not None:
						result[k] = result[k][t.span()[0]:t.span()[1]].strip()
						result[k] = result[k].replace(".", " ")
						result[k] = result[k].replace("Pass ", "Passenger ")
						result[k] = result[k].replace("Sports", "Sport")
						result[k] = result[k].replace("L L Bean", "L.L.Bean")
					else:
						result[k] = "Base"
