...
class Saab_Corrections(Correction_Template):
	"""
	Helper class for Saab corrections
	"""
		...
		for k in data.keys():
			match k:
				...
				case "Style":
					...
					s = re.search(r"Coupe|Sedan|Convetible", result[k])
					if s is not None:
						result[k] = result[k][s.span()[0]:s.span()[1]]
					else:
						if "Wagon" in result['Trim'] or "Wgn" in result['Trim']:
							result[k] = "Wagon"
						else:
							result[k] = ""
				case "Trim":
					sport = ""
					if "SportCombi" in result[k]:
						sport = "SportCombi"
					elif "Sport" in result[k]:
						sport = "Sport"

					t = re.search(r"((Comfort|Premium|Griffin|Aero|Turbo(\d|X)?|Independence) ?)+", result[k])
					if t is not None:
						result[k] = sport + " " + result[k][t.span()[0]:t.span()[1]].rstrip()
