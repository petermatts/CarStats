...
class VinFast_Corrections(Correction_Template):
	"""
	Helper class for VinFast corrections
	"""
		...
		for k in data.keys():
			match k:
				...
				case "Style":
					result[k] = ""
				case "Trim":
					result[k] = result[k].replace("AWD ", "")

					t = re.search(r"((Eco|Plus|\d\d(\.\d)?|kWh) ?)+", result[k])
					if t is not None:
