...
class Rivian_Corrections(Correction_Template):
	"""
	Helper class for Rivian corrections
	"""
		...
		for k in data.keys():
			match k:
				...
				case "Style":
					result[k] = ""
				case "Trim":
					t = re.search(r"((Adventure|Performance|Launch Edition|(Dual|Quad)-Motor|(Large|Standard|Max) Pack) ?)+", result[k])
					if t is not None:
